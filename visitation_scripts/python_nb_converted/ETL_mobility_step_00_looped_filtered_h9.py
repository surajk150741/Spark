...
####Arguments
date_start = "2022-07-16"#dbutils.widgets.get('date_start')
...
for date_start in day_values:
    ...
    def writeToLogs(str_arg):
        ...
        try:
            f = BytesIO()
            bucket_name = "ia-mobility-logs"
            logs_path = 'logs/{0}/{2}/{1}_h9filtered_pre-process.txt'.format(str(tminus.strftime('%Y-%m-%d')), country, source)
            #object_storage_client = ObjectStorageClient(signer=signer, config={})
    ...
    df3 = df3.withColumn("index_res", lit(9))

    df3 = df3.withColumn("latitude",col("latitude").cast(DoubleType())) \
        .withColumn("longitude",col("longitude").cast(DoubleType()))

    df3 = df3.withColumn("h9", h3_pyspark.geo_to_h3("latitude", "longitude", "index_res"))

    df3 = df3.filter(df3.h9.isin(["89608a2483bffff","89608b09223ffff","89608b19b37ffff"]))

    #df3.show(3)
    
    df3.count()
    save_string = f"oci://ia-customer-insights@bmmp5bv7olp2/{source.lower()}_data/hdfs_data/step_0_filtered_h9/{country}/"
    ...
    start_time = time.time()
    try:
        newDF.write.partitionBy(['date']).mode("append").format('delta').save(save_string)
    ...
    file_path =  'analytics/{1}/{0}/h9filtered_analysis.txt'.format(str(tminus.strftime('%Y-%m-%d')), source)
    ...
    print(f"completed step0 for date={date_start}")
