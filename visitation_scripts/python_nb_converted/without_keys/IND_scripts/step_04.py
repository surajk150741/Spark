...
client= "all_ind_3.3_all"#dbutils.widgets.get('client')#"Test_silver_level"
...
start_time = time.time()
#if source =="ls":
save_string = f"oci://ia-visitation-data@bmmp5bv7olp2/poiVisit/delta/{country}/all_{country.lower()}_all_poi_v3.2.2_full/"+str(indexing_level)+"/"
#else:
...
dfx2.write.partitionBy(["source","month","date"]).format("delta").mode('append').option("mergeSchema", "true").option("timestampFormat", "yyyy-MM-dd HH:mm:ss").save(save_string,header =True)
#dfx2.show(3)
...
#dfx2.count()#                                        #commented for speed
