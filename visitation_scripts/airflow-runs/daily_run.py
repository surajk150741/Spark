...
class AirflowDagRuns():
    ...
    __cookie__ = "session=26a81126-bdda-4dad-8705-9022b23ecdae.q3rnK8Gtk2TSxrfqEn2IoDzekxQ; Expires=Sun, 02 Apr 2023 09:12:40 GMT; HttpOnly; Path=/; SameSite=Lax"
    ...
    def __run_dags__(self,dag_id, dag_run_id,input_date, country):
        ...
        try:
            ...
            elif (dag_id=="visitation_step_1")  or  (dag_id=="visitation_step_4") or  (dag_id=="visitation_step_5"):
                print(input_date)
                payload = json.dumps({
                "dag_run_id": dag_run_id,
                "conf": {
...
five_dag_id = ["visitation_step_0", "visitation_step_1","visitation_step_2", "visitation_step_4","visitation_step_5"]
four_dag_id = ["visitation_step_0", "visitation_step_1","visitation_step_2", "visitation_step_4"]
three_dag_id = ["visitation_step_1","visitation_step_2", "visitation_step_4"]
two_dag_id = ["visitation_step_2", "visitation_step_4"]
one_dag_id = ["visitation_step_4"]
...
for day in date_list:
    for id in two_dag_id:
        dag_id = id
        if dag_id!="visitation_step_0":
            entered_date = dt.datetime.strptime(day, '%Y-%m-%d')
            entered_date = entered_date.date()
            date1 = entered_date - timedelta(days = 5)
            input_date = f"'{date1}'"
        else:
            input_date = f"'{day}'"
        logger.info(f"dag_run='{dag_id}';message='All date list {date_list}'")
        ar_runs = AirflowDagRuns(dag_id=dag_id)
...
end_date = date.today()
input_date = start_date - timedelta(days = 5)
input_date2 = f"'{input_date}'"
input_date3 = f"'{start_date}'"
print(type(input_date2),type(input_date3))
print(input_date2,input_date3)
'''
